//! Reading an archive back: validation first, then the writes it implies.

use crate::api::activities::ActivityInput;
use crate::api::objects::{ObjectInput, ObjectRow};
use crate::api::reminders::ReminderInput;
...
fn archive_types(data: &Export) -> Result<Vec<(String, TypeInput)>, AppError> {
    ...
    data.types
        ...
        .map(|(i, t)| {
            ...
            let uuid = crate::api::normalize_client_uuid(Some(t.client_uuid.clone()))
