...
pub(crate) async fn name_taken(conn: &mut sqlx::AnyConnection, user_id: i64, name: &str, except: Option<i64>) -> Result<bool, AppError> {
    ...
    Ok(rows.iter().any(|(id, other)| Some(*id) != except && fold(other) == wanted))
}
...
async fn create(user: AuthUser, State(state): State<App>, Json(body): Json<TypeBody>) -> Result<(StatusCode, Json<TypeOut>), AppError> {
    ...
    if let Some(uuid) = client_uuid.as_deref() {
        ...
        let existing: Option<(i64, i64, Option<String>)> =
            sqlx::query_as("SELECT id, user_id, deleted_at FROM object_types WHERE client_uuid = $1")
                .bind(uuid)
                .fetch_optional(&state.db)
                .await?;
        match existing {
            Some((id, owner, None)) if owner == user.id => {
                return Ok((StatusCode::OK, Json(load_owned(&state.db, user.id, id).await?.into())));
            }
...
async fn delete(user: AuthUser, State(state): State<App>, Path(id): Path<i64>) -> Result<StatusCode, AppError> {
    ...
    let uuid: Option<(String,)> =
        sqlx::query_as("SELECT client_uuid FROM object_types WHERE id = $1 AND user_id = $2 AND deleted_at IS NULL")
            .bind(id).bind(user.id)
            .fetch_optional(&mut *tx).await?;
    let Some((uuid,)) = uuid else { return Err(AppError::NotFound) };
    ...
    let in_use: i64 = sqlx::query_scalar(
        ...
        .bind(user.id).bind(format!("{CUSTOM_PREFIX}{uuid}"))
        .fetch_one(&mut *tx).await?;
    if in_use > 0 {
