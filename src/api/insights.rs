...
use crate::domain::insights::{
    consumption_per_100_milli, consumption_per_fill, cost_per_counter_milli, daily_rate_milli,
    default_fuel_unit, fuel_cost_per_counter_milli, latest_reading, monthly_usage, DatedFill, Fill,
    FillRate, MonthUsage, Reading, RATE_WINDOW_DAYS,
};
...
async fn read(
    ...
    let since = object
        ...
        .as_deref()
        .and_then(day_of)
        .or_else(|| day_of(&object.created_at))
        .unwrap_or(today);
    let until = object
        .archived_at
        .as_deref()
    ...
    let fuel = if fill_rows.is_empty() {
        ...
        Some(FuelOut {
            ...
            fills: consumption_per_fill(
                &fill_rows
                    .iter()
                    .map(|(date, counter, quantity_milli, _)| DatedFill {
                        date: date.clone(),
                        counter: *counter,
                        quantity_milli: *quantity_milli,
                    })
        ...
        let readings: Vec<Reading> = rows
            ...
            .filter_map(|(date, counter)| {
                NaiveDate::parse_from_str(&date, "%Y-%m-%d")
                    .ok()
                    .map(|date| Reading { date, counter })
            })
            .collect();
        let months = monthly_usage(&readings, today, USAGE_MONTHS);
        if months.iter().any(|m| m.amount.is_some()) {
            months
        } else {
            Vec::new()
        }
