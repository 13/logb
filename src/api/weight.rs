...
pub fn router() -> Router<App> {
    Router::new().route("/objects/{id}/weight", get(history)).route("/objects/{id}/weight/summary", get(summary))
}
...
async fn summary(user: AuthUser, State(state): State<App>, Path(id): Path<i64>) -> Result<Json<WeightSummary>, AppError> {
    load_owned_object(&state, user.id, id).await?;
