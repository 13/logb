...
pub fn parse_timezone(raw: &str) -> Result<Tz, AppError> {
    raw.trim().parse::<Tz>()
        .map_err(|_| AppError::BadRequest("timezone must be an IANA name like Europe/Berlin".into()))
}
...
async fn write(AdminUser(_): AdminUser, State(state): State<App>, Json(body): Json<SettingsInput>) -> Result<Json<Settings>, AppError> {
