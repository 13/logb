...
| `LOGB_TELEGRAM_BOT_TOKEN` | unset | Deprecated compatibility token for connections made before v0.13.11; new bot tokens are saved by each user in Settings |
| `LOGB_TELEGRAM_API_URL` | `https://api.telegram.org` | Telegram Bot API base; change only for a self-hosted Bot API server |
...
selected interface language. The form previews the next three fixed dates and allows a future
start. Logging a counter value (or weight for a body object) advances its reading reminder;
...
then paste its token under Settings → Notifications. LogB validates it, encrypts it with a key kept
beside the data directory, and never returns it through the API. Choose **Connect Telegram**, open
the short-lived link and press Start. Disconnecting removes only the chat association; removing the
bot also deletes its saved credential. Telegram receives the same daily digest as the webhook and
browser notifications, in the user's language. Back up `telegram.key` together with the database:
encrypted bot tokens cannot be recovered without it.
