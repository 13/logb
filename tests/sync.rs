...
async fn pushing_category_to_trip_without_a_stored_start_counter_is_rejected() {
    ...
    let body: serde_json::Value = res.json().await.unwrap();
    assert_eq!(body["results"][0]["outcome"], "rejected", "{body}");
    assert_eq!(body["results"][0]["reason"], "a trip needs start_counter and counter_value");

    let category: String = sqlx::query_scalar("SELECT category FROM activities WHERE client_uuid = $1")
        .bind(&uuid).fetch_one(&app.state.db).await.unwrap();
...
async fn pushing_category_to_trip_on_an_hour_object_is_rejected() {
    let app = common::spawn().await;
    app.setup("ben", "correct horse").await;
    let treadmill = app.client.post(app.url("/objects")).json(&json!({
    ...
    let body: serde_json::Value = res.json().await.unwrap();
    assert_eq!(body["results"][0]["outcome"], "rejected", "{body}");
    assert_eq!(body["results"][0]["reason"], "a trip needs an object that counts km or mi");
}
...
async fn pushing_counter_value_to_null_on_a_trip_is_rejected() {
    ...
    let stored: Option<i64> = sqlx::query_scalar("SELECT counter_value FROM activities WHERE client_uuid = $1")
        .bind(&uuid).fetch_one(&app.state.db).await.unwrap();
...
async fn pushing_start_counter_to_null_on_a_trip_is_rejected() {
    ...
    let stored: Option<i64> = sqlx::query_scalar("SELECT start_counter FROM activities WHERE client_uuid = $1")
        .bind(&uuid).fetch_one(&app.state.db).await.unwrap();
...
async fn create_charge(app: &common::TestApp, object_id: i64, extra: serde_json::Value) -> serde_json::Value {
...
async fn pushing_charged_full_on_a_fuel_row_is_accepted_and_stamps_the_field_clock() {
    ...
    let stored: i64 = sqlx::query_scalar("SELECT charged_full FROM activities WHERE client_uuid = $1")
        .bind(&uuid).fetch_one(&app.state.db).await.unwrap();
    assert_eq!(stored, 1);
...
async fn pushing_charged_full_on_a_non_fuel_row_is_rejected() {
    ...
    let body: serde_json::Value = res.json().await.unwrap();
    assert_eq!(body["results"][0]["outcome"], "rejected", "{body}");
    assert_eq!(body["results"][0]["reason"], "only a charge can be marked full");
...
/// The same category change is accepted once the flag is not in the way.
#[tokio::test]
...
async fn pushing_energy_price_milli_with_a_stored_fuel_unit_is_accepted() {
    let app = common::spawn().await;
    app.setup("ben", "correct horse").await;
    let car = app.client.post(app.url("/objects")).json(&json!({
    ...
    let stored: Option<i64> = sqlx::query_scalar("SELECT energy_price_milli FROM objects WHERE client_uuid = $1")
        .bind(&object_uuid).fetch_one(&app.state.db).await.unwrap();
...
async fn pushing_energy_price_milli_without_a_stored_fuel_unit_is_rejected() {
    ...
    let body: serde_json::Value = res.json().await.unwrap();
    assert_eq!(body["results"][0]["outcome"], "rejected", "{body}");
    assert_eq!(body["results"][0]["reason"], "energy_price_milli needs a fuel unit");
...
async fn pushing_fuel_unit_to_null_while_a_price_is_stored_is_rejected() {
    ...
    let unit: Option<String> = sqlx::query_scalar("SELECT fuel_unit FROM objects WHERE client_uuid = $1")
        .bind(&object_uuid).fetch_one(&app.state.db).await.unwrap();
    assert_eq!(unit.as_deref(), Some("kwh"), "the rejected write must not land");
}
