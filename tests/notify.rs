...
async fn the_digest_posts_every_due_reminder_as_json() {
    ...
    let digest = logb::notify::tick(&app.state, 9).await.unwrap().expect("a digest was due");
...
async fn the_digest_goes_out_once_a_day_and_not_before_the_configured_hour() {
    ...
    assert!(logb::notify::tick(&app.state, 7).await.unwrap().is_none(), "too early in the day");
    assert!(inbox.received().is_empty());

    assert!(logb::notify::tick(&app.state, 8).await.unwrap().is_some());
    assert!(logb::notify::tick(&app.state, 9).await.unwrap().is_none(), "already sent today");
...
async fn without_a_url_the_scheduler_does_nothing() {
    ...
    seed_overdue(&app).await;
    assert!(logb::notify::tick(&app.state, 23).await.unwrap().is_none());
    assert!(logb::notify::collect(&app.state).await.unwrap().is_some(), "reminders really are due");
}
...
async fn a_failing_endpoint_is_not_retried_until_tomorrow() {
    ...
    assert!(logb::notify::tick(&app.state, 9).await.is_err(), "the failure is reported");
    assert!(logb::notify::tick(&app.state, 9).await.unwrap().is_none(), "but the day is done");
}
...
async fn a_failure_while_collecting_does_not_burn_the_day() {
    ...
    assert!(logb::notify::tick(&app.state, 9).await.is_err(), "collect must surface its failure");
    assert!(inbox.received().is_empty(), "nothing was sent");

    sqlx::query("ALTER TABLE reminders_hidden RENAME TO reminders")
        .execute(&app.state.db).await.unwrap();
    let digest = logb::notify::tick(&app.state, 9).await.unwrap()
