...
async fn add_activity(app: &common::TestApp, object_id: i64, body: serde_json::Value) -> serde_json::Value {
...
async fn reading_reminder(app: &common::TestApp, object_id: i64, body: serde_json::Value) -> serde_json::Value {
    let res = app.client.post(app.url(&format!("/objects/{object_id}/reminders"))).json(&body).send().await.unwrap();
    assert_eq!(res.status(), 201, "{body} -- {}", res.text().await.unwrap());
    res.json().await.unwrap()
}
...
async fn car(app: &common::TestApp) -> i64 {
    ...
    app.create_object(&app.client, "Golf", Some("km")).await["id"].as_i64().unwrap()
}
...
async fn a_reading_reminder_is_validated_as_its_own_kind() {
    ...
    let home = app.create_object(&app.client, "Home", None).await["id"].as_i64().unwrap();

    for (object, body) in [
        (home, json!({ "title": "Meter", "kind": "reading", "every_n": 1, "every_unit": "month" })),
        (id, json!({ "title": "Km", "kind": "reading" })),
        (id, json!({ "title": "Km", "kind": "reading", "every_n": 0, "every_unit": "month" })),
        (id, json!({ "title": "Km", "kind": "reading", "every_n": 1, "every_unit": "day" })),
        (id, json!({ "title": "Km", "kind": "reading", "every_n": 1, "every_unit": "month", "due_counter": 5 })),
        (id, json!({ "title": "Km", "kind": "reading", "every_n": 1, "every_unit": "month", "repeat_months": 1 })),
        (id, json!({ "title": "Oil", "due_date": "2030-01-01", "every_n": 1, "every_unit": "month" })),
        (id, json!({ "title": "Km", "kind": "odometer", "every_n": 1, "every_unit": "month" })),
    ] {
        let res = app.client.post(app.url(&format!("/objects/{object}/reminders"))).json(&body).send().await.unwrap();
        assert_eq!(res.status(), 400, "{body}");
    }

    // Leaving the start out means "from today".
    let r = reading_reminder(&app, id, json!({ "title": "Km", "kind": "reading", "every_n": 1, "every_unit": "month" })).await;
    ...
    let res = app.client.patch(app.url(&format!("/reminders/{}", r["id"])))
        .json(&json!({ "title": "Km", "due_date": "2030-01-01" })).send().await.unwrap();
    ...
    assert_eq!((r["every_n"].as_i64(), r["every_unit"].as_str()), (Some(2), Some("week")));
}
...
async fn any_entry_with_a_counter_clears_it_and_deleting_that_entry_brings_it_back() {
    ...
    assert_eq!(get(&app, &format!("/objects/{id}")).await["stats"]["due_reminder_count"], 1);

    // An entry without a counter says nothing about the odometer.
    add_activity(&app, id, json!({ "date": today().to_string(), "category": "repair", "title": "Wipers" })).await;
    ...
    assert_eq!(r["next_due_date"], today().checked_add_months(Months::new(1)).unwrap().to_string());
    ...
    assert_eq!(get(&app, &format!("/reminders/{rid}")).await["due"], true, "the reading it rested on is gone");
    assert_eq!(get(&app, &format!("/objects/{id}")).await["stats"]["due_reminder_count"], 1);
}
...
async fn an_old_reading_does_not_count_and_a_future_dated_one_is_ignored() {
    ...
    assert_eq!(get(&app, &format!("/reminders/{rid}")).await["due"], true, "a month has passed since");
...
async fn a_reading_is_logged_with_a_value_and_its_reminder_is_never_marked_done() {
    ...
    let r = reading_reminder(&app, id, json!({ "title": "Log km", "kind": "reading", "every_n": 1, "every_unit": "month" })).await;
    let res = app.client.post(app.url(&format!("/reminders/{}/done", r["id"]))).json(&json!({})).send().await.unwrap();
...
async fn skipping_a_due_reading_hides_it_until_the_snooze_lapses() {
    ...
    let res = app.client.post(app.url(&format!("/reminders/{rid}/snooze"))).json(&json!({ "days": 30 })).send().await.unwrap();
    assert_eq!(res.status(), 200, "{}", res.text().await.unwrap());
    let r: serde_json::Value = res.json().await.unwrap();
    assert_eq!(r["due"], false);
    // Overdue, so the skip runs from today -- not from 2020.
    assert_eq!(r["snoozed_until"], today().checked_add_days(Days::new(30)).unwrap().to_string());
    assert_eq!(get(&app, &format!("/objects/{id}")).await["stats"]["due_reminder_count"], 0);
}
...
async fn usage_estimates_when_a_mileage_service_comes_due() {
    ...
    let service = app.client.post(app.url(&format!("/objects/{id}/reminders")))
        .json(&json!({ "title": "Service", "due_counter": 14_000 })).send().await.unwrap();
    ...
    assert_eq!(service["due"], false, "an estimate never makes a reminder due");

    let far = app.client.post(app.url(&format!("/objects/{id}/reminders")))
        .json(&json!({ "title": "Timing belt", "due_counter": 90_000 })).send().await.unwrap();
    let far: serde_json::Value = far.json().await.unwrap();

    let soon: Vec<serde_json::Value> = serde_json::from_value(get(&app, "/reminders/due?within_days=30").await).unwrap();
    let ids: Vec<&serde_json::Value> = soon.iter().map(|r| &r["id"]).collect();
    assert!(ids.contains(&&service["id"]), "a mileage-only service now shows up as coming up: {soon:#?}");
    assert!(!ids.contains(&&far["id"]), "4 years out is not coming up");
    assert!(serde_json::from_value::<Vec<serde_json::Value>>(get(&app, "/reminders/due").await).unwrap().is_empty());
}
...
async fn a_reading_reminder_survives_export_and_import() {
    ...
    let zip = app.client.get(app.url("/export")).send().await.unwrap().bytes().await.unwrap().to_vec();
    let anna = app.create_user_client("anna", "password123").await;
    let res = anna.post(app.url("/import")).header("content-type", "application/zip").body(zip).send().await.unwrap();
    assert_eq!(res.status(), 200, "{}", res.text().await.unwrap());
    ...
    let acts: serde_json::Value = anna.get(app.url(&format!("/objects/{nid}/activities"))).send().await.unwrap().json().await.unwrap();
    ...
    let app = Router::new().route(
        ...
        post(move |headers: axum::http::HeaderMap, body: String| {
            ...
            async move {
                ...
                sink.0.lock().unwrap().push((header("title"), header("click"), body));
                "ok"
            }
        }),
    );
...
async fn the_digest_lists_readings_under_their_own_heading_with_a_link() {
    ...
    logb::notify::tick(&app.state, 9).await.unwrap().expect("a digest was due");
    ...
    assert_eq!(got[0].1, link, "one reminder: tapping the notification opens its form");
...
async fn services_come_first_and_readings_follow() {
    ...
    app.client.post(app.url(&format!("/objects/{id}/reminders")))
        .json(&json!({ "title": "Oil change", "due_date": "2020-01-01" })).send().await.unwrap();
    reading_reminder(&app, id, json!({
        "title": "Log km", "kind": "reading", "every_n": 1, "every_unit": "month", "due_date": "2020-01-01"
    })).await;
    ...
    assert!(digest.reminders.iter().all(|r| r.link.is_none()), "no public URL, no links");
}
