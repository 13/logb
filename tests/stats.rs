...
async fn household_energy_groups_kwh_by_month() {
    ...
    let kwh = object(
        &app,
        json!({ "name": "Bike", "type": "e_bike", "counter_unit": "km", "fuel_unit": "kwh" }),
    )
    .await;
    let home = object(&app, json!({ "name": "Home battery", "type": "appliance", "fuel_unit": "kwh", "counter_unit": "h" })).await;
    let petrol = object(
        &app,
        json!({ "name": "Car", "type": "car", "fuel_unit": "l", "counter_unit": "km" }),
    )
    .await;
    ...
    let future = (today + chrono::Days::new(2))
        .format("%Y-%m-%d")
        .to_string();
    ...
    assert_eq!(
        out["months"].as_array().unwrap().last().unwrap()["charges"],
        2
    );
    assert_eq!(
        out["months"].as_array().unwrap().last().unwrap()["objects"],
        2
    );
}
...
async fn household_fuel_keeps_litres_and_gallons_separate() {
    ...
    let litres = object(
        &app,
        json!({ "name": "Oil tank", "type": "home", "fuel_unit": "l", "counter_unit": "h" }),
    )
    .await;
    let gallons = object(
        &app,
        json!({ "name": "Generator", "type": "other", "fuel_unit": "gal", "counter_unit": "h" }),
    )
    .await;
...
#[tokio::test]
async fn household_fuel_reports_the_latest_tank_level_and_remaining_volume() {
    let app = common::spawn().await;
    app.setup("ben", "correct horse").await;
    ...
    let tank = object(
        &app,
        json!({
            "name": "Heating oil", "type": "home", "fuel_unit": "l", "counter_unit": "h",
            "fuel_capacity_milli": 2_000_000
        }),
    )
    .await;
    for (date, level) in [("2026-01-01", 80), (today.as_str(), 35)] {
        let res = app.client.post(app.url(&format!("/objects/{tank}/activities")))
    ...
    let out = app.get_json("/stats/fuel").await;
    assert_eq!(out["levels"][0]["object_id"], tank);
    assert_eq!(out["levels"][0]["level_pct"], 35);
    ...
    let bad = app
        ...
        .post(app.url("/objects"))
        .json(&json!({
            "name": "Electricity", "type": "home", "fuel_unit": "kwh", "fuel_capacity_milli": 10_000
        }))
        .send()
        .await
        .unwrap();
...
#[tokio::test]
async fn household_water_combines_meter_differences_and_period_usage() {
    let app = common::spawn().await;
    app.setup("ben", "correct horse").await;
    let today: NaiveDate = logb::db::today().parse().unwrap();
    let previous = today
    ...
    let meter = object(
        &app,
        json!({
            "name": "Water meter", "type": "home", "resource_kind": "water", "resource_unit": "m3",
            "measurement_mode": "meter", "monthly_target_milli": 4_000
        }),
    )
    .await;
    for (date, reading) in [(previous.as_str(), 100_000), (current.as_str(), 102_500)] {
        let res = app
            .client
            .post(app.url(&format!("/objects/{meter}/activities")))
            .json(&json!({
                "date": date, "category": "usage", "title": "", "meter_reading_milli": reading
            }))
            .send()
            .await
            .unwrap();
    ...
    let billed = object(
        &app,
        json!({
            "name": "Garden water", "type": "home", "resource_kind": "water", "resource_unit": "l",
            "measurement_mode": "usage"
        }),
    )
    .await;
    let res = app
        .client
        .post(app.url(&format!("/objects/{billed}/activities")))
        .json(&json!({
            "date": current, "category": "usage", "title": "", "quantity_milli": 500_000,
            "period_start": today.with_day(1).unwrap().format("%Y-%m-%d").to_string(),
            "period_end": today.format("%Y-%m-%d").to_string(), "cost_cents": 1200, "estimated": 1
        }))
        .send()
        .await
        .unwrap();
    ...
    let out = app.get_json("/stats/water").await;
    assert_eq!(out["current_liters_milli"], 3_000_000);
    assert_eq!(out["current_cost_cents"], 1200);
    assert_eq!(
        out["months"].as_array().unwrap().last().unwrap()["estimated"],
        true
    );
    let meter_row = out["objects"]
        .as_array()
        .unwrap()
        .iter()
        .find(|o| o["object_id"] == meter)
        .unwrap();
    assert_eq!(meter_row["target_liters_milli"], 4_000_000);
}
...
fn buckets(v: &Value) -> Vec<(String, i64)> {
    v.as_array()
        ...
        .map(|b| {
            (
                b["bucket"].as_str().unwrap().to_string(),
                b["cost_cents"].as_i64().unwrap(),
            )
        })
        .collect()
}
...
async fn seed(app: &common::TestApp) -> (i64, i64, i64) {
    let house = object(
        app,
        json!({ "name": "House", "type": "home", "description": "",
        "purchase_date": "2024-05-01", "purchase_price_cents": 300_000 }),
    )
    .await;
    let boiler = object(
        app,
        json!({ "name": "Boiler", "type": "appliance", "description": "", "parent_id": house }),
    )
    .await;
    let car = object(
        app,
        json!({ "name": "Car", "type": "car", "counter_unit": "km", "description": "" }),
    )
    .await;
...
async fn all_years_roll_children_into_their_parent() {
    ...
    assert_eq!(
        buckets(&out["over_time"]),
        [("2025".into(), 100_000), ("2026".into(), 75_000)]
    );
...
async fn a_year_gives_twelve_months_and_keeps_the_full_year_list() {
    ...
    assert_eq!(
        out["over_time"][1],
        json!({ "bucket": "2026-02", "cost_cents": 25_000 })
    );
    assert_eq!(
        out["over_time"][0],
        json!({ "bucket": "2026-01", "cost_cents": 0 })
    );
    assert_eq!(out["years"], json!(["2026", "2025"]));
    assert_eq!(
        buckets(&out["by_category"]),
        [
            ("repair".into(), 42_000),
            ("maintenance".into(), 25_000),
            ("fuel".into(), 8_000)
        ]
    );
}
...
async fn purchases_add_their_own_row_unless_a_purchase_entry_already_has_the_cost() {
    ...
    assert!(
        buckets(&out["by_category"]).contains(&("purchase_price".into(), 300_000)),
        "only the house's price"
    );
}
...
async fn deleted_rows_are_gone_archived_objects_stay() {
    ...
    let fuel_id = fuel
        ...
        .unwrap()
        .iter()
        .find(|a| a["category"] == "fuel")
        .unwrap()["id"]
        .as_i64()
        .unwrap();
    assert!(app
        .client
        .delete(app.url(&format!("/activities/{fuel_id}")))
        .send()
        .await
        .unwrap()
        .status()
    ...
    assert!(app
        .client
        .delete(app.url(&format!("/objects/{house}")))
        .send()
        .await
        .unwrap()
        .status()
    ...
    let out = app.get_json("/stats").await;
    assert_eq!(
        out["total_cents"], 42_000,
        "a deleted house takes its boiler with it"
    );
}
...
async fn a_years_purchase_price_lands_in_its_own_month() {
    ...
    assert_eq!(
        out["total_cents"], 300_000,
        "the house's purchase price only"
    );
    assert_eq!(out["over_time"].as_array().unwrap().len(), 12);
    assert_eq!(
        out["over_time"][4],
        json!({ "bucket": "2024-05", "cost_cents": 300_000 })
    );
    assert_eq!(out["years"], json!(["2026", "2025", "2024"]));
}
...
async fn by_type_buckets_carry_custom_keys() {
    ...
    let kick = object(
        &app,
        json!({ "name": "Kick", "type": key, "description": "" }),
    )
    .await;
