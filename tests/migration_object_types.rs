...
async fn old_schema_with_rows() -> SqlitePool {
    ...
    for file in [
        "0001_init.sql",
        "0002_session_expiry_index.sql",
        "0003_fuel_quantity.sql",
        "0004_reminder_snooze.sql",
        "0005_client_op_id.sql",
        "0006_api_tokens.sql",
        "0007_sync.sql",
        "0008_sync_epoch.sql",
    ] {
...
async fn run_after_0009(pool: &SqlitePool) {
    let mut later: Vec<_> = std::fs::read_dir("migrations/sqlite")
        .unwrap()
...
async fn the_reading_migration_moves_nothing_else() {
    ...
    let attachments: i64 = sqlx::query_scalar("SELECT count(*) FROM attachments")
        ...
        .unwrap();
    let done: Option<i64> =
        sqlx::query_scalar("SELECT done_activity_id FROM reminders WHERE id = 1")
            .fetch_one(&pool)
            .await
            .unwrap();
    let kind: String = sqlx::query_scalar("SELECT kind FROM reminders WHERE id = 1")
        .fetch_one(&pool)
        .await
        .unwrap();
    ...
    assert_eq!(
        kind, "service",
        "every existing reminder is a service reminder"
    );
    ...
    assert!(
        violations.is_empty(),
        "{} foreign key violations after the rebuild",
        violations.len()
    );
...
async fn known_categories_become_types() {
    let pool = old_schema_with_rows().await;
    run_0009(&pool).await;
    let rows = sqlx::query("SELECT id, type FROM objects ORDER BY id")
        .fetch_all(&pool)
        .await
        .unwrap();
...
async fn unmapped_text_is_kept_in_the_description() {
    let pool = old_schema_with_rows().await;
    run_0009(&pool).await;
    let d: String = sqlx::query_scalar("SELECT description FROM objects WHERE id = 3")
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(
        d, "Gravelbike Custom",
        "an empty description takes the text alone"
    );
    let d: String = sqlx::query_scalar("SELECT description FROM objects WHERE id = 4")
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(
        d, "already here\nRennrad",
        "an existing description keeps its text, on its own line"
    );
}
...
async fn the_new_categories_are_accepted_and_nonsense_is_not() {
    ...
    .execute(&pool)
    .await
    .expect("symptom is a category now");
    let bad = sqlx::query(
        "INSERT INTO activities (object_id, date, category, title, created_at, updated_at) \
    ...
    assert!(
        bad.is_err(),
        "the CHECK must still reject an unknown category"
    );
}
...
async fn the_schema_accepts_every_type_and_category_the_code_offers() {
    ...
    for t in logb::object_type::OBJECT_TYPES.iter() {
        ...
        .await
        .unwrap_or_else(|e| {
            panic!("the objects CHECK rejects the type {t:?} the picker offers: {e}")
        });
    }
    ...
    for c in logb::api::activities::CATEGORIES.iter() {
        ...
        .await
        .unwrap_or_else(|e| {
            panic!("the activities CHECK rejects the category {c:?} the form offers: {e}")
        });
    }
...
async fn the_indexes_survive() {
    ...
    assert_eq!(
        names,
        vec!["idx_activities_object_date", "idx_objects_user"]
    );
}
...
async fn the_sync_uniqueness_indexes_survive() {
    ...
    assert_eq!(
        names,
        vec![
            "idx_activities_client_op",
            "idx_activities_uuid",
            "idx_objects_uuid"
        ]
    );

    sqlx::query("UPDATE objects SET client_uuid = 'u1' WHERE id = 1")
        .execute(&pool)
        .await
        .unwrap();
    let dup = sqlx::query("UPDATE objects SET client_uuid = 'u1' WHERE id = 2")
        .execute(&pool)
        .await;
    assert!(dup.is_err(), "two objects must not share a client_uuid");

    sqlx::query("UPDATE activities SET client_op_id = 'op1' WHERE id = 1")
        .execute(&pool)
        .await
        .unwrap();
    ...
    assert!(
        dup.is_err(),
        "a replayed client_op_id must not create a second activity"
    );
}
...
async fn the_sync_epoch_is_rotated() {
    let pool = old_schema_with_rows().await;
    let before: String = sqlx::query_scalar("SELECT value FROM settings WHERE key = 'sync_epoch'")
        .fetch_one(&pool)
        .await
        .unwrap();
    run_0009(&pool).await;
    let after: String = sqlx::query_scalar("SELECT value FROM settings WHERE key = 'sync_epoch'")
        .fetch_one(&pool)
        .await
        .unwrap();
...
async fn every_legacy_word_maps_through_the_real_migration() {
    ...
    assert_eq!(
        seeded.len(),
        5 * logb::object_type::LEGACY.len(),
        "every word gets every spelling"
    );
...
async fn field_clocks_for_the_dropped_column_are_removed_and_others_survive() {
    ...
    assert_eq!(
        category_clock, 0,
        "the clock for the dropped column must be gone"
    );
    ...
    assert_eq!(
        name_clock, 1,
        "a clock for a surviving column must not be swept up with it"
    );
}
