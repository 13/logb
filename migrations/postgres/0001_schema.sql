...
-- SQLite has one writer. PostgreSQL does not give that for free: an identity column hands out
-- numbers before commit and keeps them across a rollback, and concurrent transactions commit
-- out of order -- so a device can read past a number that has not committed and never see it
-- again.
--
-- Two things together make it behave, and they do different jobs. Every write transaction
-- takes the advisory lock in `db::begin_write`, which is what puts the numbers in commit
-- order; the application then assigns `seq` itself as `MAX(seq) + 1` (see
-- `sync::record::insert_change`), which is what keeps them gapless -- a number a rolled-back
-- transaction computed goes back with it rather than being burnt.
--
-- The identity stays as a backstop rather than being dropped, so an insert that forgets `seq`
-- gets a number instead of failing the NOT NULL a PRIMARY KEY implies, and `BY DEFAULT` rather
-- than `ALWAYS` is what lets the application's own value through. It is only a backstop: the
-- sequence behind it does not advance when a value is supplied, so a number it fell back on
-- would collide with one already assigned. Every insert into this table must name `seq`.
