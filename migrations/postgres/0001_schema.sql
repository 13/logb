...
-- SQLite has one writer. PostgreSQL does not give that for free, and an identity column is not
-- the end state: it hands out numbers that survive a rollback, and concurrent transactions can
-- commit out of order, so a device can read past a number that has not committed and never see
-- it again. Part two of this project assigns `seq` under an advisory lock.
--
-- It is an identity column in the meantime because nothing in the app ever binds a `seq` --
-- SQLite's `INTEGER PRIMARY KEY AUTOINCREMENT` fills it in. Declared as a plain BIGINT, every
-- write that records a change, which is every write, failed on the NOT NULL that a PRIMARY KEY
-- implies. `BY DEFAULT` rather than `ALWAYS` so the advisory-lock assignment can supply its own
-- value without dropping the default first.
