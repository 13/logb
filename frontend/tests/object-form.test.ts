...
describe('object form', () => {
  it('maps an object to editable input', () => {
    expect(toInput(obj)).toEqual({
      ...
      purchase_date: '2020-03-01', purchase_price_cents: 1500000, archived: false, parent_id: null,
    });
  ...
  it('starts empty, defaulting to the catch-all type rather than the first in the list', () => {
    expect(emptyInput()).toEqual({ name: '', type: 'other', counter_unit: null, fuel_unit: null, description: '', purchase_date: null, purchase_price_cents: null, archived: false, parent_id: null });
  });
