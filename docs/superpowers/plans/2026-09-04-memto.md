# memto Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build memto, a self-hosted web app that keeps the complete history (activities, costs, photos, documents, reminders) of owned objects.
...
Spec: `docs/superpowers/specs/2026-09-04-memto-design.md`

## Global Constraints

- Backend crate name `memto`, edition 2021, binary `memto`.
...
- Session cookie name `memto_session`, `HttpOnly`, `SameSite=Lax`, `Secure` per `MEMTO_SECURE_COOKIE` (`auto|true|false`, default `auto` = Secure when `X-Forwarded-Proto: https`). Session TTL 30 days.
- Login rate limit: 10 attempts per minute per client IP, response 429. The client IP is the socket peer address by default; only when `MEMTO_TRUST_PROXY=true` is the first `X-Forwarded-For` hop used instead. (Decided during execution, overriding the original "always trust `X-Forwarded-For`": with no proxy in front, a spoofed header per request bypasses the limiter entirely.)
- Env config: `MEMTO_DATA_DIR=./data`, `MEMTO_BIND=0.0.0.0`, `MEMTO_PORT=8080`, `MEMTO_MAX_UPLOAD_MB=50`, `MEMTO_SECURE_COOKIE=auto`, `MEMTO_TRUST_PROXY=false`, `MEMTO_LOG=info`.
- File layout: `data/memto.db`, `data/files/<sha256[0..2]>/<sha256>`, `data/thumbs/<file_id>.jpg` (400 px longest side, JPEG q80, EXIF-rotated).
...
**Interfaces:**
- Produces: `memto::build(config: Config) -> Result<axum::Router, BoxError>`; `memto::config::Config`; `memto::state::{AppState, App}`; `memto::db::{connect, now, today}`; `memto::error::AppError` with variants `BadRequest(String)`, `Unauthorized`, `Forbidden`, `NotFound`, `Conflict(String)`, `TooLarge`, `TooManyRequests`, `Db(sqlx::Error)`, `Io(std::io::Error)`, `Internal(String)`; test helper `common::spawn() -> TestApp { base, client }`.
...
Note: `max_age` takes a `time::Duration` (the `time` crate is a direct dependency, added in Task 1). Verified output: `memto_session=tok; HttpOnly; SameSite=Lax; Path=/; Max-Age=2592000`.
...
`tests/common/mod.rs` needs no change (it calls `memto::build`).
...
services:
  memto:
    ...
    environment:
      ...
      MEMTO_SECURE_COOKIE: "auto"
      MEMTO_TRUST_PROXY: "false"
      MEMTO_LOG: "info"
```
...
| `MEMTO_TRUST_PROXY`   | `false`   | take the client IP for rate limiting from `X-Forwarded-For`. Enable only behind a reverse proxy that overwrites the header |
...
**Interfaces:**
- The suite runs against the **real binary** serving the built SPA: `cargo build` then start `target/debug/memto` with `MEMTO_DATA_DIR` pointing at a scratch directory that Playwright wipes before the run, `MEMTO_PORT=8099`.
...
| Container | `docker build -t memto:dev .` |
