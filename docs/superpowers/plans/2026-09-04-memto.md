...
- Login rate limit: 10 attempts per minute per client IP (`X-Forwarded-For` first hop, else socket address), response 429.
- Env config: `MEMTO_DATA_DIR=./data`, `MEMTO_BIND=0.0.0.0`, `MEMTO_PORT=8080`, `MEMTO_MAX_UPLOAD_MB=50`, `MEMTO_SECURE_COOKIE=auto`, `MEMTO_LOG=info`.
...
- [ ] **Step 3: Write config, state, db, error modules**
...
- [ ] **Step 4: Write lib.rs, api/mod.rs, domain/mod.rs, main.rs**
...
- [ ] **Step 5: Write the test harness and the health test**
...
- [ ] **Step 4: Implement src/api/auth.rs and mount**
...
- [ ] **Step 3: Implement users and settings**
...
const SELECT: &str = "SELECT id, username, is_admin, lang, created_at FROM users";
...
(Remove the `SELECT` const and the `let _ = SELECT;` line if you prefer; they are only there so the three identical SELECTs can be replaced by `&format!("{SELECT} WHERE id = ?")` — sqlx 0.9 requires `sqlx::AssertSqlSafe(..)` for non-literal SQL, so keeping literals is simpler.)
...
- [ ] **Step 3: Implement src/api/objects.rs**
...
- [ ] **Step 3: Implement src/api/activities.rs**
...
const COLS: &str = "id, object_id, date, category, title, notes, counter_value, cost_cents, created_at, updated_at";
...
(`COLS` is unused; delete it and the `let _ = COLS;` line — sqlx 0.9 wants literal SQL, so the column list is repeated.)
...
- [ ] **Step 1: Write domain unit tests and the failing API tests**
...
- [ ] **Step 3: Implement src/api/reminders.rs**
...
const SELECT: &str = "SELECT r.id, r.object_id, r.title, r.notes, r.due_date, r.due_counter, r.repeat_months, \
...
(`SELECT` const is documentation of the shared shape; delete it and `let _ = SELECT;` if the unused warning bothers you.)
...
- [ ] **Step 1: Write unit tests for files.rs and the failing API tests**
...
- [ ] **Step 4: Wire storage into state and lib**
...
- [ ] **Step 5: Implement src/api/attachments.rs**
...
- [ ] **Step 6: Attach attachments to activity responses and purge on deletes**
...
- [ ] **Step 3: Implement src/api/export.rs**
...
- [ ] **Step 3: Implement src/spa.rs and wire it**
...
- [ ] **Step 5: Write Docker and build files**
...
- [ ] **Step 6: Verify the Docker build compiles (optional if Docker is slow; required before release)**
...
- [ ] **Step 4: Implement the library modules**
...
- [ ] **Step 6: App shell, styles, TopBar, Setup, Login, Dashboard placeholder**
...
- [ ] **Step 7: Type-check, build, and verify the embedded SPA**
...
- [ ] **Step 3: Implement the helper and the screens**
...
- [ ] **Step 4: Implement FilePicker, Documents, Timeline**
...
- [ ] **Step 5: Implement ObjectDetail and ActivityForm**
...
- [ ] **Step 4: Implement Reminders.svelte and ReminderForm.svelte**
...
- [ ] **Step 1: Implement Settings.svelte**
...
- [ ] **Step 1: Write the Playwright config and helpers**
