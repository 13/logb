# logb Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build logb, a self-hosted web app that keeps the complete history (activities, costs, photos, documents, reminders) of owned objects.
...
Spec: `docs/superpowers/specs/2026-09-04-logb-design.md`

## Global Constraints

- Backend crate name `logb`, edition 2021, binary `logb`.
...
- Session cookie name `logb_session`, `HttpOnly`, `SameSite=Lax`, `Secure` per `LOGB_SECURE_COOKIE` (`auto|true|false`, default `auto` = Secure when `X-Forwarded-Proto: https`). Session TTL 30 days.
- Login rate limit: 10 attempts per minute per client IP, response 429. The client IP is the socket peer address by default; only when `LOGB_TRUST_PROXY=true` is the first `X-Forwarded-For` hop used instead. (Decided during execution, overriding the original "always trust `X-Forwarded-For`": with no proxy in front, a spoofed header per request bypasses the limiter entirely.)
- Env config: `LOGB_DATA_DIR=./data`, `LOGB_BIND=0.0.0.0`, `LOGB_PORT=8080`, `LOGB_MAX_UPLOAD_MB=50`, `LOGB_SECURE_COOKIE=auto`, `LOGB_TRUST_PROXY=false`, `LOGB_LOG=info`.
- File layout: `data/logb.db`, `data/files/<sha256[0..2]>/<sha256>`, `data/thumbs/<file_id>.jpg` (400 px longest side, JPEG q80, EXIF-rotated).
...
**Interfaces:**
- Produces: `logb::build(config: Config) -> Result<axum::Router, BoxError>`; `logb::config::Config`; `logb::state::{AppState, App}`; `logb::db::{connect, now, today}`; `logb::error::AppError` with variants `BadRequest(String)`, `Unauthorized`, `Forbidden`, `NotFound`, `Conflict(String)`, `TooLarge`, `TooManyRequests`, `Db(sqlx::Error)`, `Io(std::io::Error)`, `Internal(String)`; test helper `common::spawn() -> TestApp { base, client }`.
...
async fn main() -> Result<(), logb::db::BoxError> {
    ...
    let app = logb::build(config).await?;
...
pub async fn spawn() -> TestApp {
    ...
    let app = logb::build(config).await.unwrap();
...
Note: `max_age` takes a `time::Duration` (the `time` crate is a direct dependency, added in Task 1). Verified output: `logb_session=tok; HttpOnly; SameSite=Lax; Path=/; Max-Age=2592000`.
...
`tests/common/mod.rs` needs no change (it calls `logb::build`).
...
| `LOGB_TRUST_PROXY`   | `false`   | take the client IP for rate limiting from `X-Forwarded-For`. Enable only behind a reverse proxy that overwrites the header |
...
JSON under `/api`, cookie session. See `docs/superpowers/specs/2026-09-04-logb-design.md`.
...
**Interfaces:**
- The suite runs against the **real binary** serving the built SPA: `cargo build` then start `target/debug/logb` with `LOGB_DATA_DIR` pointing at a scratch directory that Playwright wipes before the run, `LOGB_PORT=8099`.
...
export default defineConfig({
  ...
  webServer: {
    command: '../target/debug/logb',
    env: { LOGB_DATA_DIR: '../.e2e-data', LOGB_PORT: '8099', LOGB_BIND: '127.0.0.1', LOGB_LOG: 'warn' },
    port: 8099,
...
| Container | `docker build -t logb:dev .` |
